...
- **Builds de prueba sin gastar versión (tags `beta-N`):** `.github/workflows/build-installers.yml` también dispara con tags `beta-1`, `beta-2`, … (contador plano, sin `X.Y.Z` propio — evita acoplar el intento a la versión candidata y evita el problema de que Microsoft Store exige que el 4º segmento de versión del MSIX sea `0`).
  ...
  - La versión que llevan los artefactos (instalador Windows, manifiesto MSIX, `.deb`) sigue viniendo de `VERSION` en la raíz del repo (fuente de verdad única del proyecto, propagada a `APP_RELEASE_NAME`, web y `pubspec.yaml` por `scripts/sync_shared_assets.py`; ver más abajo), no del nombre del tag — no hace falta repetir el número de versión al crear el tag beta.
...
- **Versión unificada (obligatorio no editar a mano):** `VERSION` en la raíz del repo es la **única** fuente de verdad del número de versión para escritorio, web y móvil. No editar directamente `APP_RELEASE_NAME` (`midichords/core/app_constants.py`), `APP_VERSION` (`apps/web/worker/_worker.js`), `WEB_APP_VERSION_FALLBACK` (`apps/web/static/app.js`), `softwareVersion` (`apps/web/app.html`/`index.html`) ni `version: X.Y.Z+N` (`apps/mobile_flutter/pubspec.yaml`): son derivados que `python scripts/sync_shared_assets.py` regenera desde `VERSION`. `launch.py` (cualquier `run_*`) y `scripts/check.py` lo invocan automáticamente antes de ejecutar; `--check` falla si algo diverge (útil en CI). El build number de Flutter (`+N`) se autoincrementa solo cuando cambia la versión base — no lo edites manualmente.
...
1. Actualizar `VERSION` en la raíz (si cambia la versión base) y ejecutar `python scripts/sync_shared_assets.py`, que propaga a `apps/mobile_flutter/pubspec.yaml` (`version: X.Y.Z+N`) incrementando `N` automáticamente. No editar `pubspec.yaml` a mano.
