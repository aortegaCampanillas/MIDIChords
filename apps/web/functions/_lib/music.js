...
function voicedIntervalsForInversion(intervals, inversion) {
  ...
  for (const raw of rotated) {
    let value = raw;
    if (voiced.length) {
      while (value <= voiced[voiced.length - 1]) value += 12;
    }
    voiced.push(value);
  }
  return voiced;
...
function listScalePatterns(_language = "es") {
...
function generateChord({ rootPc = 0, suffix = "", inversion = 0, language = "es", preferFlat = false }) {
  ...
  if (!selected || !selected.intervals.length) {
    return { root_pc: Number(rootPc) % 12, suffix: "", inversion: 0, name: "-", notes_midi: [], notes: [] };
  }
