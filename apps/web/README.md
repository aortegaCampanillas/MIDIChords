...
El cliente es una **SPA** en **`static/app.js`** y **`static/style.css`**, cargada desde **`app.html`**. La lógica pura, los catálogos y los adaptadores verificables se reparten entre módulos enlazados antes de `app.js`: textos, notación, círculo, intervalos, digitaciones, armaduras, escalas, salida MIDI, audio por samples, matemática del afinador y ayudas. `app.js` conserva estado, DOM, permisos, entrada MIDI, gesto, bus y creación de nodos Web Audio y renderizado. El selector de modo (`#modeSelect`) alterna entre: detección de acordes, **detección de intervalos**, generación de acordes, **círculo de quintas**, escalas, metrónomo y afinador.
