...
| Versión de producto | `VERSION` (raíz) | `scripts/sync_shared_assets.py` propaga a `APP_RELEASE_NAME` (`midichords/core/app_constants.py`), `APP_VERSION` (worker web), `WEB_APP_VERSION_FALLBACK` (`apps/web/static/app.js`), `softwareVersion` (`apps/web/app.html`/`index.html`) y `version: X.Y.Z+N` (`apps/mobile_flutter/pubspec.yaml`, incrementando `N` automáticamente) | `scripts/sync_shared_assets.py --check`; `launch.py` y `scripts/check.py` lo invocan automáticamente antes de ejecutar |
